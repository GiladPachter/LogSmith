...
Enforces strict hierarchy rules and prevents duplicate logger creation.

## Core Members

```
- name                       — logger’s name.
- level                      — logger’s current log level (inherits when NOTSET).
...
- raw                        — write unformatted text directly to handlers (sanitized unless disabled).
- retire()                   — close handlers and disable the logger.
- destroy()                  — remove logger entirely from the logging system.
```
...
- error
- critical
- raw                     — pure-text message-only log entry (no metadata fields)
...
Requires normalized absolute paths for file handlers.
...
- a_error
- a_critical
- a_raw                   — pure-text message-only log entry (no metadata fields)
- <dynamic level methods> — programmatically added log levels.
```
...
- terminate_auditing     — disable async auditing.
```
...
- message_parts_order          — ordering of inline metadata fields (timestamp always first, message always last).
...
Diagnostics fields (`exc_info`, `stack_info`) must **not** appear in `message_parts_order`.
...
```
- RAINBOW / FIRE / OCEAN / FOREST / SUNSET / PASTEL / NEON / GREYSCALE
...
- large_entry_behavior        — behavior when a single entry exceeds maxBytes.
- create_handler              — internal factory for rotation‑aware handlers.
```
...
```
- Seconds / Minutes / Hours / Days / MonthDay  — retention scale.
...
```
- timestamp / level  
- relative_created / logger_name / file_path / file_name / lineno / func_name
- thread_id / thread_name / task_name / process_id / process_name
- named_args               — structured fields
