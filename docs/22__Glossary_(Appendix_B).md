...
Async Rotation             — Rotation performed in a thread pool via asyncio.to_thread().
Audit Log                  — A global log capturing all log events from all loggers.
Audit Handler              — Handler used to collect all log events during auditing.
Audit Formatter            — Strict formatter used by audit handlers (plain or NDJSON).
```
...
Bleaching (Color Bleaching)  — Recoloring non‑ANSI text to console default while preserving ANSI segments.
```
...
Color Theme          — Mapping of level numbers to LevelStyle objects.
...
Critical Level       — Highest built‑in severity level (CRITICAL = 50).
```
...
Fields (Structured Fields)  — Named arguments passed to log methods (stored under named_args).
FileHandlerRegistry         — Global registry preventing duplicate file handlers.
...
Hierarchy Validation — Enforcement of parent‑exists and type‑matching rules for logger creation.
...
# J
```
JSON Output — Structured JSON representation of log entries (pretty‑printed).
```
...
# L
```
LargeLogEntryBehavior  — Defines how rotation handles entries larger than maxBytes.
...
Preserve Colors in Files   — Option allowing ANSI codes to remain in file output.
...
# R
```
Raw Output          — Unformatted text written directly to handlers (sanitized unless disabled).
...
Theme                    — Mapping of level numbers to LevelStyle objects.
...
# U
```
User Fields — Custom structured fields provided via named arguments (named_args).
```
...
Worker Debounce — Mechanism preventing redundant async rotation scheduling.
```
