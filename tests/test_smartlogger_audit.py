...
def test_sync_audit_mirroring(tmp_path):
    ...
    SmartLogger._SmartLogger__audit_handler = audit._SmartLogger__py_logger.handlers[0]

    main.info("hello audit sync")

    # flush both
    for h in main._SmartLogger__py_logger.handlers:
        h.flush()
    for h in audit._SmartLogger__py_logger.handlers:
        h.flush()
